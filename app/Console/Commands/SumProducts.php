	...
	public function handle() {
		...
		foreach ($data as $value) {
            ...
            if($consTabla->where('consecutivo_factura','<>',$value['consecutivo_factura'])->where(['FECHA' => $value['FECHA'], 'CODIGO_CLIENTE' => $value['CODIGO_CLIENTE'], 'ZONA' => $value['ZONA'], 'CODIGO_PRODUCTO' => $value['CODIGO_PRODUCTO']])->exists()){
				...
				// $consTabla->where(['codigo' => $value['codigo'], 'CODIGO_CLIENTE' => $value['CODIGO_CLIENTE']])->update(['PEDIDO' => $contPedido, 'DESPACHADO' => $contDespachado,'CAMBIOS' => $contCambios]);

				if($consTabla->where(['codigo' => $value['codigo'], 'CODIGO_CLIENTE' => $value['CODIGO_CLIENTE']])->update(['PEDIDO' => $contPedido, 'DESPACHADO' => $contDespachado,'CAMBIOS' => $contCambios])){
					echo "=====> [ ACTUALIZANDO PRIMER REGISTRO DUPLICADO: ".$value['consecutivo_factura']." ] <===== \n";
					if($consTabla->where('codigo','=',$dataF['codigo'])->delete()){
						echo "====> SE ELIMINO EL REGISTRO ".$dataF['codigo']." \n";
					}
				}else{
